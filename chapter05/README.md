...
- 인터럽트 신호의 속성
	- 인터럽트를 인지하는 방식에 따라 크게 에지-트리커 인터럽트와 레벨-트리커 인터럽트로 분류
	- 에지-트리거 인터럽트는 전기 신호가 어떤 상태에서 다른 상태로(보통은 하이(high) -> 로우(low)로) 넘어가는 순간 인터럽트를 감지하는 방식
	- 레벨-트리커 인터럽트는 인터럽트 라인을 하이(high)로 유지할 때 인터럽트로 식별
- 커널에서는 인터럽트 신호를 식별하는 방식에 따라 인터럽트를 설정하도록 플래그를 정의
- 인터럽트 신호를 식별하는 방식
	- IRQF_TRIGGER_RISING 플래그: 인터럽트 신호가 LOW에서 HIGH로 올라갈 때 (pull down을 하지 않으면,, 굉장히 찾기 어려운 버그 발생 -> 인터럽트가 아닌데 인터럽트로 식별하는 경우가 있다)
	- IRQF_TRIGGER_FALLING 플래그: 인터럽트 신호가 HIGH에서 LOW로 떨어질 때
